...
logger.info('cargando un imputador y generando el primer model...')
model = Pipeline([
    ('imputer', SimpleImputer(strategy='mean',missing_values=np.nan)),
    ('core_model', GradientBoostingRegressor())
])

logger.info('SEPARANDO LOS DATOS X y Y')
X = data.drop(['worldwide_gross'], axis= 1)
y = data['worldwide_gross']
logger.info('VALORES DE PRUEBA Y ENTRENAMIENTO...')
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.35, random_state=42)
logger.info('GENERANDO HYPERPARAMETROS')
param_tuning = {'core_model__n_estimators':range(20,301,20)}
logger.info('EVALUANDO HYPERPARAMETROS')
grid_search = GridSearchCV(model, param_grid= param_tuning, scoring='r2', cv=5)


logger.info('AJUSTANDO EL MODELO...')
grid_search.fit(X_train, y_train)

logger.info('EVALUANDO EL MODELO model...')
...
logger.info('GUARDANDO EL MODELO, FUNCION DE UTILS...')
update_model(grid_search.best_estimator_)

logger.info('REPORTE BASICO CON FUNCION DE UTILS...')
...
logger.info('GRAFICA BASICA CON FUNCION DE UTILS...')
